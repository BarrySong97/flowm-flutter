...
class SyncAnalyzer {
  ...
  static Future<SyncDecision> analyzeSyncNeed({
    ...
    print('[SyncAnalyzer] 开始分析同步需求');
    print('[SyncAnalyzer] 本地哈希: $currentLocalHash');
    print('[SyncAnalyzer] 远程信息: $currentRemoteInfo');
    print('[SyncAnalyzer] 上次同步记录: $lastSyncRecord');
    ...
    if (remoteAnchor?.type == SyncAnchorType.hash && 
        ...
        syncService != null) {
      print('[SyncAnalyzer] 远程 ETag 不可用，降级到 Hash 计算');
      try {
        final remoteData = await syncService.webdavClient
            .downloadFile(syncService.remoteDatabasePath);
        final remoteHash = await syncService.calculateFileHashFromBytes(remoteData);
        remoteAnchor = SyncAnchor.fromHash(remoteHash);
        print('[SyncAnalyzer] 计算得到远程哈希: $remoteHash');
      } catch (e) {
        print('[SyncAnalyzer] 下载远程文件计算哈希失败: $e');
        // 无法获取远程哈希，按照网络错误处理
        remoteAnchor = null;
      }
    }
    ...
    print('[SyncAnalyzer] 本地是否变化: $localChanged');
    print('[SyncAnalyzer] 远程是否变化: $remoteChanged');
    
    // 5. 特殊情况处理
    // 5.1 如果没有有效的同步记录，按照首次同步处理
    if (!lastSyncRecord.hasValidRecord) {
      print('[SyncAnalyzer] 无有效同步记录，按首次同步处理');
    ...
    if (currentRemoteInfo == null || remoteAnchor == null) {
      if (localAnchor != null) {
        print('[SyncAnalyzer] 网络连接问题，建议上传本地数据');
        return SyncDecision.upload;
      } else {
        print('[SyncAnalyzer] 网络连接问题，无本地数据，无需同步');
    ...
    if (!localChanged && !remoteChanged) {
      print('[SyncAnalyzer] 双方都没变化，无需同步');
      return SyncDecision.noSync;
    } else if (localChanged && !remoteChanged) {
      print('[SyncAnalyzer] 仅本地变化，需要上传');
      return SyncDecision.upload;
    } else if (!localChanged && remoteChanged) {
      print('[SyncAnalyzer] 仅远程变化，需要下载');
      return SyncDecision.download;
    } else {
      print('[SyncAnalyzer] 双方都有变化，存在冲突');
  ...
  static SyncAnchor? _getLastRemoteAnchor(SyncRecord record) {
    if (record.lastRemoteEtag != null && ETagUtils.isValidEtag(record.lastRemoteEtag)) {
  ...
  /// 分析首次同步情况
  static SyncDecision _analyzeFirstTimeSync(SyncAnchor? local, SyncAnchor? remote) {
