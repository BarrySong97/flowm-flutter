...
class AccountTransactionGuideScreen extends StatelessWidget {
  ...
  Widget _buildCommonTransactionsCard() {
    final commonTransactions = [
      _TransactionExample('日常消费', '资产 → 费用', '银行存款 → 餐饮费用', '用银行卡支付餐费', Colors.green),
      _TransactionExample('收入入账', '收入 → 资产', '工资收入 → 银行存款', '工资到账', Colors.green),
      _TransactionExample('信用卡消费', '负债 → 费用', '信用卡 → 购物费用', '用信用卡购物', Colors.green),
      _TransactionExample('还信用卡', '资产 → 负债', '银行存款 → 信用卡', '用储蓄卡还信用卡', Colors.green),
      _TransactionExample('资产转移', '资产 → 资产', '现金 → 银行存款', '现金存入银行', Colors.green),
      _TransactionExample('借款入账', '负债 → 资产', '借款 → 银行存款', '借到的钱到账', Colors.green),
    ];
  ...
  Widget _buildUncommonTransactionsCard() {
    final uncommonTransactions = [
      _TransactionExample('投资增资', '资产 → 权益', '银行存款 → 股票投资', '用存款买股票', Colors.orange),
      _TransactionExample('提取投资', '权益 → 资产', '股票投资 → 银行存款', '卖股票获得现金', Colors.orange),
      _TransactionExample('债务转移', '负债 → 负债', '信用卡A → 信用卡B', '用信用卡A还信用卡B', Colors.orange),
      _TransactionExample('收入还债', '收入 → 负债', '工资收入 → 信用卡', '工资直接还信用卡', Colors.orange),
      _TransactionExample('费用退款', '费用 → 资产', '餐饮费用 → 银行存款', '餐费退款到账', Colors.orange),
    ];
  ...
  Widget _buildAbnormalTransactionsCard() {
    final abnormalTransactions = [
      _TransactionExample('异常组合', '资产 → 收入', '银行存款 → 工资收入', '资产不会转入收入账户', Colors.red),
      _TransactionExample('异常组合', '收入 → 费用', '工资收入 → 餐饮费用', '应该是资产支付费用', Colors.red),
      _TransactionExample('异常组合', '权益 → 费用', '个人投资 → 购物费用', '权益不会直接支付费用', Colors.red),
      _TransactionExample('异常组合', '费用 → 负债', '餐饮费用 → 信用卡', '费用不会转入债务', Colors.red),
      _TransactionExample('异常组合', '收入 → 收入', '工资收入 → 奖金收入', '收入账户内部转移不合理', Colors.red),
    ];
...
class _TransactionExample {
  ...
  _TransactionExample(this.title, this.pattern, this.example, this.description, this.color);
}