...
class GlobalRefreshService {
  ...
  static Future<void> refreshDatabaseFile(WidgetRef ref) async {
    ...
    try {
      AppLogger.debug(
          '[GlobalRefreshService] 开始强制数据库文件刷新 - ${startTime.toIso8601String()}');
      ...
      AppLogger.debug(
          '[GlobalRefreshService] 数据库文件刷新完成，耗时: ${dbRefreshDuration.inMilliseconds}ms');
      ...
      AppLogger.debug(
          '[GlobalRefreshService] 强制数据库文件刷新总耗时: ${totalDuration.inMilliseconds}ms');
    } catch (e, stackTrace) {
      final errorDuration = DateTime.now().difference(startTime);
      AppLogger.debug(
          '[GlobalRefreshService] 强制数据库文件刷新失败，耗时: ${errorDuration.inMilliseconds}ms，错误: $e');
      AppLogger.debug('[GlobalRefreshService] 错误堆栈: $stackTrace');

      // 如果失败，回退到完整刷新
      AppLogger.debug('[GlobalRefreshService] 回退到完整数据刷新');
      refreshAllData(ref);
    }
  }
  ...
  static void refreshDataLight(WidgetRef ref) {
    ...
    try {
      AppLogger.debug(
          '[GlobalRefreshService] 开始轻量级数据刷新 - ${startTime.toIso8601String()}');
      ...
      final invalidateDuration =
          invalidateEndTime.difference(invalidateStartTime);

      AppLogger.debug(
          '[GlobalRefreshService] 轻量级刷新完成，耗时: ${invalidateDuration.inMilliseconds}ms');

      final totalDuration = DateTime.now().difference(startTime);
      AppLogger.debug(
          '[GlobalRefreshService] 轻量级数据刷新总耗时: ${totalDuration.inMilliseconds}ms');
    } catch (e, stackTrace) {
      final errorDuration = DateTime.now().difference(startTime);
      AppLogger.debug(
          '[GlobalRefreshService] 轻量级刷新失败，耗时: ${errorDuration.inMilliseconds}ms，错误: $e');
      AppLogger.debug('[GlobalRefreshService] 错误堆栈: $stackTrace');
  ...
  static void refreshAllData(WidgetRef ref) {
    ...
    try {
      AppLogger.debug(
          '[GlobalRefreshService] 开始全局数据刷新 - ${startTime.toIso8601String()}');

      // 1. 触发数据库刷新（最关键的步骤）
      final dbRefreshStartTime = DateTime.now();
      final currentValue = ref.read(databaseRefreshTriggerProvider);
      ref.read(databaseRefreshTriggerProvider.notifier).state =
          currentValue + 1;
      final dbRefreshEndTime = DateTime.now();
      final dbRefreshDuration = dbRefreshEndTime.difference(dbRefreshStartTime);

      AppLogger.debug(
          '[GlobalRefreshService] 数据库连接刷新触发器已更新: ${currentValue + 1}，耗时: ${dbRefreshDuration.inMilliseconds}ms');
      ...
      AppLogger.debug(
          '[GlobalRefreshService] 核心 providers 已刷新，耗时: ${invalidateDuration.inMilliseconds}ms');
      ...
      final businessProviderDuration =
          businessProviderEndTime.difference(businessProviderStartTime);

      AppLogger.debug(
          '[GlobalRefreshService] 业务 providers 已刷新，耗时: ${businessProviderDuration.inMilliseconds}ms');

      final totalDuration = DateTime.now().difference(startTime);
      AppLogger.debug(
          '[GlobalRefreshService] 全局数据刷新完成，总耗时: ${totalDuration.inMilliseconds}ms');
    } catch (e, stackTrace) {
      final errorDuration = DateTime.now().difference(startTime);
      AppLogger.debug(
          '[GlobalRefreshService] 数据刷新过程中发生错误，耗时: ${errorDuration.inMilliseconds}ms，错误: $e');
      AppLogger.debug('[GlobalRefreshService] 错误堆栈: $stackTrace');
      rethrow;
    }
  }
  ...
  static Future<void> refreshAllDataAsync(WidgetRef ref) async {
    final startTime = DateTime.now();
    AppLogger.debug(
        '[GlobalRefreshService] 开始异步全局数据刷新 - ${startTime.toIso8601String()}');
    ...
    final totalDuration = DateTime.now().difference(startTime);
    AppLogger.debug(
        '[GlobalRefreshService] 异步全局数据刷新完成，总耗时: ${totalDuration.inMilliseconds}ms');
  }
}
