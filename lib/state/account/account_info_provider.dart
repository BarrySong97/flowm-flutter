...
/// 用于支出和收入详情页面的动态数据获取
final accountExpenseNodeProvider =
    FutureProvider.family<AccountExpenseNode, int>((ref, accountId) async {
  final repository = ref.watch(accountRepositoryProvider);
...
final accountTreeInfoProvider =
  ...
  if (account.accountType == AccountType.ASSET) {
    try {
      ...
      AppLogger.debug(
          'Warning: Could not find account $accountId in asset tree: $e');
      // 继续执行，只是没有子账户信息
    }
  } else if (account.accountType == AccountType.LIABILITY) {
    try {
      final liabilityTree = await repository.getLiabilityAccountTree(
          ledgerId: selectedLedger.ledgerId);
      ...
      AppLogger.debug(
          'Warning: Could not find account $accountId in liability tree: $e');
      // 继续执行，只是没有子账户信息
    }
  }
